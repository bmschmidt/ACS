...
``` {r}
Industries = persons %.% filter(SCHL>=21,AGEP < 60,AGEP>20,WKHP>30) %.% select(PINCP,AGEP,PWGTP,FOD1P,SEX,OCCP10,OCCP12) %.% collect()

industries = Industries %.% encode("OCCP10") %.% encode("OCCP12") %.% encode("FOD1P") %.% mutate(weight=PWGTP,occupation=factor(ifelse(is.na(OCCP10),as.character(OCCP12),as.character(OCCP10))))
...
expectations = industries %.% ungroup() %.% 
  ...
  group_by(field) %.% 
  mutate(majorCount=sum(weight),majorShare=majorCount/total) %.% 
  group_by(occupation,add=F) %.% 
  mutate(industryCount=sum(weight),industryShare=industryCount/total) %.%
  ungroup() %.% 
  group_by(field,occupation) %.% summarize(count = sum(PWGTP),expected=total[1]*industryShare[1]*majorShare[1],diff=count/expected)


expectations %.% arrange(-count) %.% filter(field=="ENGLISH LANGUAGE AND LITERATURE")

tops = expectations %.% filterToTopN("field",countVariable=quote(count)) %.% filterToTopN("occupation",countVariable=quote(count),n=45)
write.table(tops,file="/tmp/tmp.tsv",sep="\t",quote=F,row.names=F)
